...
> Save `.excalidraw` files under `artifacts/diagrams/` (create it if missing)
> rather than the repository root. Diagrams that will ship on
> digeratiexperts.com follow `design/DIAGRAM_SYSTEM.md` and the Visual System
> v2 classification (LIVE / SANITIZED REAL / EXAMPLE / ILLUSTRATIVE); an
> Excalidraw sketch is working material, not a production primitive. For a
> rendered hand-drawn PNG instead of an editable file, use
> `.claude/skills/excalidraw-visuals/`. No API key or network call is involved.
...
1. Save to `artifacts/diagrams/[concept-slug].excalidraw` (see the repository note above)
