...
> Skills in this repository follow the vendored-skill convention: each skill
> directory carries an `UPSTREAM.md` (source, audit date, local deviations,
> external-service boundary), keeps scripts and assets inside its own folder
> rather than the repository's operational `scripts/`, reads API keys only from
> the environment or the gitignored `.env`, and writes generated files under
> `artifacts/`. List new skills in the "Project skills" section of `CLAUDE.md`.
> Governance in `AGENTS.md` and `docs/AI-ENGINEERING-GOVERNANCE.md` still applies
> to any skill that touches website code.
