---
name: trigger-dev
description: Build and deploy TypeScript automations on Trigger.dev (SDK v4) for beginners: research, clarify, plan, build tasks under src/trigger/, set env vars, test with the dev server, deploy through the Trigger.dev MCP. Use when someone asks to automate a process, build a scheduled job, cron task, poller, or workflow on Trigger.dev, or mentions trigger.dev tasks.
---

> **Digerati Experts repository note (local addition, not upstream text).**
> Supplied by Joe as a project `CLAUDE.md` ("Claude Workflow Builder") for a
> Trigger.dev project. Vendored here as a reference skill; this repository's
> `CLAUDE.md` and `AGENTS.md` remain the governance pointers. Read it as
> follows here:
>
> - **This website repo is not a Trigger.dev project.** It has no
>   `@trigger.dev/sdk` dependency and no `src/trigger/`. Build automations in a
>   dedicated Trigger.dev repository or project. Adding Trigger.dev to this
>   repo would be a new governance claim (issue first), not a side effect of
>   an automation request.
> - **Deploy rule does not transfer.** The "push to `master` and GitHub
>   Actions auto-deploys" line below describes the author's Trigger.dev repo.
>   In this repository the CI workflow deploys the public website to the
>   production VPS on `main`; never push here to deploy an automation, and
>   never push to `main` at all (PR-only, per governance).
> - **Secrets.** `.env` / `.env.*` are gitignored here. Trigger.dev dashboard
>   env vars are set by Joe; never paste keys into issues, PRs or logs.
> - **MCP.** The Trigger.dev MCP server config is in the repository root
>   `.mcp.json` (`npx trigger.dev@4.4.0 mcp`); Claude Code asks before
>   enabling a project MCP server, and it needs a Trigger.dev login.
> - **Reference.** `/trigger-ref` is installed as its own skill
