...
**FFmpeg and FFprobe must be on PATH** (check with `ffmpeg -version` and `ffprobe -version`). If they are missing, tell the user which package to install for their OS instead of downloading binaries yourself. Build video-to-website pages as isolated experiments (for example under `scrollcraft/builds/<name>/`), never inside `client/`.
