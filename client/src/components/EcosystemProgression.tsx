...
export function EcosystemProgression({ compact = false, detailed = false }: EcosystemProgressionProps) {
  return (
    ...
    >
      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#D3126A]">
        ProActive Ecosystem
      </p>
      ...
      <ol className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {pricingTiers.map((tier, index) => (
          ...
          >
            ...
            <p className="mt-1 text-sm text-white/50">{formatUserPrice(tier.id)}</p>
            {detailed && (
              <p className="mt-1 text-xs text-white/40">
                {formatPrice(tier.monthlyMin)} monthly minimum
              </p>
            )}
            <p className="mt-3 flex-1 text-sm leading-relaxed text-white/60">{fit[tier.id]}</p>
            {detailed && (
            ...
            <Link href={tier.learnMoreUrl}>
              <span className="mt-5 inline-flex min-h-11 items-center gap-1 text-sm font-semibold text-[#D3126A] transition-colors hover:text-[#f0187a] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D3126A]/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--de-bg)]">
                {tier.label}
