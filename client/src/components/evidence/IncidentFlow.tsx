...
/**
 * Educational scenarios only. They intentionally avoid vendor-specific actions,
 * measured response times, event counts, and claims of guaranteed outcomes.
 * Real incidents must be passed in separately and classified SANITIZED_REAL.
 */
const defaultScenarios: ScenarioDefinition[] = [
  {
    ...
    steps: [
      {
        label: "Signal reviewed",
        detail: "Available identity and session context is reviewed to determine whether the activity needs containment.",
        actor: "HUMAN_REVIEW",
      },
      {
        label: "Access contained as appropriate",
        detail: "Sessions, credentials, or access paths can be restricted according to the client environment and approved operating procedure.",
        actor: "CONTROL",
      },
      {
        label: "Related exposure checked",
        detail: "Relevant audit history and account settings are reviewed for additional indicators that require follow-up.",
        actor: "WORKFLOW",
      },
      {
        label: "Follow-up documented",
        detail: "Required remediation, user actions, and validation steps are recorded for the client team.",
        actor: "HUMAN_REVIEW",
      },
    ],
    outcome: "The access path is addressed and the required validation and follow-up actions are documented.",
  },
  {
    ...
    steps: [
      {
        label: "Endpoint signal triaged",
        detail: "Process, device, and available security context is reviewed to determine severity and scope.",
        actor: "HUMAN_REVIEW",
      },
      {
        label: "Device contained when required",
        detail: "The affected endpoint can be isolated or otherwise restricted when the deployed controls and client policy support that action.",
        actor: "CONTROL",
      },
      {
        label: "Recovery path selected",
        detail: "Remediation may include removal, restoration, rebuild, credential follow-up, or another documented recovery step based on findings.",
        actor: "WORKFLOW",
      },
    ],
  ...
  },
  {
    ...
    steps: [
      {
        label: "Message context reviewed",
        detail: "Sender, destination, authentication, and available security signals are evaluated using the controls present in the environment.",
        actor: "HUMAN_REVIEW",
      },
      {
        label: "Exposure reduced",
        detail: "The message, link, session, or related access can be restricted using the available controls when appropriate.",
        actor: "CONTROL",
      },
      {
        label: "Related activity checked",
        detail: "Relevant accounts and audit information are reviewed to identify follow-up work that may be necessary.",
        actor: "WORKFLOW",
      },
    ],
        ...
        {scenarios.map((scenario, index) => {
          ...
          return (
            <button
              ...
              className={`min-h-11 rounded-lg border px-3 py-2 font-mono text-xs font-semibold transition-colors ${
                isSelected
                  ? "border-[#D3126A] bg-[#D3126A] text-white"
                  : "border-de-hairline bg-de-bg text-white/65 hover:border-white/20 hover:text-white"
              }`}
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="rounded-lg border border-de-hairline bg-de-bg p-4">
            <p className="font-mono text-xs font-semibold uppercase tracking-wider text-[#F04C97]">
              Example vector / {activeScenario.category}
            </p>
            <h5 className="mt-2 font-heading text-lg font-semibold text-white">
            ...
            <ol className="space-y-3">
              {activeScenario.steps.map((step, stepIdx) => (
                <li key={`${activeScenario.id}-${step.label}`} className="grid grid-cols-[2rem_1fr] gap-3">
                  <span className="mt-1 flex h-7 w-7 items-center justify-center rounded-full border border-[#D3126A]/35 bg-de-bg font-mono text-[10px] font-bold text-[#F04C97]">
                    {String(stepIdx + 1).padStart(2, "0")}
                  </span>
                  <div className="rounded-lg border border-de-hairline bg-de-bg/70 p-4">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <p className="font-semibold text-white">{step.label}</p>
                      <span className="rounded border border-de-hairline bg-white/[0.03] px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider text-white/50">
          ...
          <div className="flex items-start gap-3 rounded-lg border border-[#D3126A]/25 bg-[#D3126A]/[0.06] p-4">
            ...
            <div>
              <span className="font-mono text-[10px] font-bold uppercase tracking-wider text-[#F04C97]">
