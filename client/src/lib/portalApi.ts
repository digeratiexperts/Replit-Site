...
/** Redirect to canonical portal login (never //login). */
export function redirectToPortalLogin(returnTo?: string) {
  if (typeof window === "undefined") return;
  const path = window.location.pathname || "";
  if (path === PORTAL_LOGIN || path.startsWith(`${PORTAL_LOGIN}?`)) return;
  ...
  const safeReturn =
    dest.startsWith("/portal") && !dest.startsWith("//") && dest !== PORTAL_LOGIN
      ? dest
      : "/portal/dashboard";
...
export async function portalFetch(url: string, options: RequestInit = {}) {
  ...
  if (response.status === 401 && typeof window !== "undefined") {
    const path = window.location.pathname || "";
    if (path.startsWith("/portal") && path !== PORTAL_LOGIN) {
