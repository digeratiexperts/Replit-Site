...
describe("Door 2 public leakage", () => {
  ...
  it("covers 13 families with both delivery models and no email gate copy", () => {
    ...
    expect(familyPage).toContain('["standalone", "Standalone"]');
    expect(familyPage).toContain('["co_managed", "Co-managed"]');
    expect(familyPage).toContain("Request this solution");
    ...
    expect(familyPage).not.toContain("useCart");
  });
});
