...
- **Checkout Flow**: Zoho Payments integration for payments (`/api/store/checkout/zoho`), Quote Request option for contract discussions
...
- **Zoho Payments**: Primary payment processor (requires ZOHO_PAYMENTS_API_KEY and ZOHO_PAYMENTS_SIGNING_KEY); checkout via `/api/store/checkout/zoho`, webhook at `/api/webhooks/zoho-payments`
