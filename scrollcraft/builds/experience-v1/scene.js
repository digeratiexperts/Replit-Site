...
function gradientTexture(stops) {
  ...
  const g = c.getContext('2d'); const grad = g.createLinearGradient(0, 128, 0, 0);
...
export function createWorld(canvas, opts = {}) {
  ...
  /* floor, walls, window */
  const floorMat = new THREE.MeshStandardMaterial({ color: C.floor, roughness: 0.92, metalness: 0.05, transparent: true, opacity: 1 });
  ...
  [[-0.72, 1.1, 4.55, 0.06, 2.2, 0.06], [0.72, 1.1, 4.55, 0.06, 2.2, 0.06], [0, 2.2, 4.55, 1.5, 0.06, 0.06]].forEach(([x, y, z, w, h, d]) => {
  ...
  const deskMat = new THREE.MeshStandardMaterial({ color: C.desk, roughness: 0.55, metalness: 0.15 });
  const legMat = new THREE.MeshStandardMaterial({ color: 0x110e1c, roughness: 0.8 });
  const standMat = new THREE.MeshStandardMaterial({ color: 0x0c0a14, roughness: 0.7 });
  const desks = DESKS.map(([x, z], i) => {
    ...
    const base = new THREE.Mesh(new THREE.BoxGeometry(1.5, 0.7, 0.1), legMat); base.position.set(0, 0.36, -0.3); g.add(base);
    const stand = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.22, 0.12), standMat); stand.position.set(0, 0.86, -0.22); g.add(stand);
    const screenMat = new THREE.MeshBasicMaterial({ color: C.screen, transparent: true, opacity: 0.0 });
    const screen = new THREE.Mesh(new THREE.PlaneGeometry(0.58, 0.35), screenMat); screen.position.set(0, 1.13, -0.2); screen.rotation.x = -0.1; g.add(screen);
    const bezel = new THREE.Mesh(new THREE.BoxGeometry(0.62, 0.39, 0.02), standMat); bezel.position.set(0, 1.13, -0.212); bezel.rotation.x = -0.1; g.add(bezel);
  ...
  const PEOPLE = [
    ...
    { seat: 4, x: 0.15, z: 5.6, y: 1.06, walker: true }, // P4 walks in and sits at desk 5 (near-middle)
  ];
  const people = PEOPLE.map((p) => {
    const m = new THREE.Mesh(new THREE.CapsuleGeometry(0.2, 0.56, 6, 12), personMat); m.position.set(p.x, p.y, p.z); m.userData = p; world.add(m);
  ...
  /* ----------------------------------------------------- camera keyframes */
  const KEYS = [
    [0.0, [0.5, 1.5, 6.6], [0.0, 1.1, -1.0]],
    [0.9, [0.9, 1.65, 3.9], [0.2, 1.0, -1.4]],
    [1.35, [3.2, 3.5, 5.8], [0.2, 1.2, -0.6]],
    [1.95, [5.7, 6.5, 7.6], [0.0, 1.3, -0.4]],
    [2.45, [5.7, 6.5, 7.6], [0.0, 1.3, -0.4]],
    [3.05, [5.0, 5.7, 6.7], [0.0, 1.3, -0.3]],
    [3.95, [6.4, 7.4, 8.6], [0.0, 1.3, -0.3]],
  ];
  ...
  function update(t, dt = 0.016) {
    ...
    const tilt = -1.2 * (Math.PI / 180) * disorder;    // heading off true, then level
    ...
    people.forEach((m) => {
      ...
      const ring = p.ring; ring.position.set(m.position.x, m.position.y + 0.72, m.position.z);
    ...
    desks.forEach((g, i) => {
      const on = ramp(t, 0.04 + i * 0.05, 0.14 + i * 0.05);
      const flick = t < 0.5 ? 0.85 + 0.15 * Math.abs(Math.sin(idleClock * 23 + i * 7)) : 1;
      g.userData.screen.opacity = on * flick * lerp(1, 0.85, glass);
    ...
    vaultMat.opacity = lerp(0, 0.28, ramp(t, 1.2, 1.4)); vaultEdges.material.opacity = lerp(0, 0.6, ramp(t, 1.2, 1.4));
    ...
    chains.forEach((c, i) => {
      ...
      const top = v.set(person.position.x, person.position.y + 0.42, person.position.z).clone();
    ...
    netT.forEach((n, i) => { n.th.set(deskScreenAnchor(i), swAnchor); n.th.setProgress(ramp(t, 1.24 + i * 0.02, 1.36 + i * 0.02) * lerp(n.gap, 1, close)); n.th.mat.color.setHex(C.thread).lerp(new THREE.Color(C.ink), settle * 0.5); });
    ...
    sun.intensity = lerp(2.4, 1.3, exposed) + settle * 1.0;
    ambient.intensity = 0.28 + 0.15 * exposed;
    deLight.intensity = 9 * de * (1 - settle * 0.7); deLight.position.set(people[0].position.x + 1.5, 2.4, people[0].position.z - 0.5);
    ...
    anchors.applications = slabAnchor(1).add(new THREE.Vector3(0, 0.4, 0));
    ...
    anchors.de = people[0].userData.ring.position.clone().add(new THREE.Vector3(0.9, 0.5, 0));
