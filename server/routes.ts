...
<body>
  ...
  // POST /api/store/checkout/zoho and POST /api/store/orders are registered
  // earlier (server/index.ts -> registerSecureZohoStoreCheckout, called before
  // registerRoutes) with server-authoritative pricing/status handlers in
  // server/secureStoreCheckout.ts. Express uses the first-registered handler
  // for a given path, so the versions that used to live here were dead code
  // (confirmed unreachable) and have been removed as a hygiene cleanup --
  // see docs/MASTER-GUARDRAILS.md and MASTER-STATUS.md, 2026-08-25.
